...
Assert-True ($machineState -notmatch "ENABLE_RELEASE") "MachineState: le test piston ne doit plus relacher l'enable de station (etat arme permanent)."
Assert-True ($machineState -match "OUTPUT_RELEASE") "MachineState: le test piston doit relacher la sortie apres l'impulsion."
Assert-True ($machineState -match "un seul aller-retour") "MachineState: le test piston doit etre un aller-retour unique."
